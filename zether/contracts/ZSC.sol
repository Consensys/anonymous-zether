...
contract ZSC {
    ...
    constructor(address _coin, uint256 _epochLength, uint256 _chainId) public {
    ...
    function rollOver(bytes32 yHash) internal { // internal, so y is already hashed...
        uint256 e = block.number / E;
    ...
    function fund(bytes32[2] calldata y, uint256 bTransfer) external {
        ...
        // pain in the ass won't let me assign this array using literals / casts
        assembly {
            let m := mload(0x40)
            mstore(m, 0x01)
    ...
    function transfer(bytes32[2] calldata outL, bytes32[2] calldata inL, bytes32[2] calldata inOutR, bytes32[2] calldata y, bytes32[2] calldata yBar, bytes calldata proof, bytes32[3] calldata signature) external {
        ...
        rollOver(yBarHash); // this step is missing from Zether! could cause honest transfers to fail
        // let's say I'm transferring; during proofgen, I will "simulate" a roll-over offline, beginning by retrieving my own acc/pTransfer.
        // if someone else transfers to me between my retrieval of my own pTransfers and the time my proof reaches the contract,
        // then they better roll ME over first. otherwise, my subsequent roll-over will fold in their new pTransfer too, and will disagree with mine
        ...
        zkp.verifyTransfer(scratch[0], scratch[1], outL, inL, inOutR, y, yBar, proof);
        verifyTransferSignature(yHash, yBar, inL, outL, inOutR, signature);
    ...
    function burn(bytes32[2] calldata y, uint256 bTransfer, bytes calldata proof, bytes32[3] calldata signature) external {
        ...
        assembly {
            ...
            mstore(m, mload(mload(scratch)))
            mstore(add(m, 0x20), mload(add(mload(scratch), 0x20)))
            mstore(add(m, 0x40), 0x01) // g_x
            mstore(add(m, 0x60), 0x02) // g_y
        ...
        zkp.verifyBurn(scratch[0], scratch[1], y, bTransfer, proof);
